...
plot_module_plotly <- function(output.summary,PFN,subset.module = NULL,col.names,
                               gene.set = NULL,color.code = "logFC",show.legend = TRUE,
                               label.hubs.only = TRUE,hubLabel.col = "red",hubLabel.sizeProp = 0.5,show.topn.hubs = 10,
                               node.sizeProp = 13,label.sizeProp = 13,label.scaleFactor = 10,label.alpha = 0.5,
                               layout = "kamada.kawai",output.plot = TRUE,out.dir = "modulePlot")
  ...
  for (i in 1:length(subset.module))
  {
    ...
    if (!is.null(gene.set) & do.set.color)
    {
      ...
      pnet[[i]] <- ggplot() + geom_segment(aes(x=X1, y=Y1, xend = X2, yend = Y2),data=edges, size = 0.5, colour="grey",
                                           alpha = 0.5) +
        geom_point(data=node.features,mapping = aes(x = X1,y = X2,colour = node.stat,size = node.size * 40,shape = node.shape)) +
        geom_text(data = node.features, aes(x = X1, y = X2, label = node.lab), size = 3) +
        scale_shape_manual(values = c("gene" = 16,"child.hub" = 18,"module.hub" = 17)) + 
        scale_colour_manual(values = c("NA" = "grey",node.stat.map)) +
        labs(title = subset.module[i]) + 
        theme_bw() + 
...
# Function hierarchy with colours
plot_module_hierarchy_coloured <- function(module.table, final_df, color_list,
                                  plot.coord = NULL,
                                  edge.color = "grey", node.label.color = "black",
                                  label.scaleFactor = 0.5, node.scaleFactor = 0.2, arrow.size = 0.015)
{
  # Add Module_Color based on Total_Genes_in_Cluster
  final_df$Module_Color <- ifelse(final_df$Total_Genes_in_Cluster < 500, color_list["Small Cluster"],
                                  ifelse(final_df$Total_Genes_in_Cluster > 50, color_list["Big Cluster"],
                                         color_list["Cluster without Genes of Interest"]))
  
  # Assign colors from final_df to module.table
  module.table$Module_Color <- final_df$Module_Color[match(module.table$id, final_df$Module)]
  
  # Handle cases where there is no match
  module.table$Module_Color[is.na(module.table$Module_Color)] <- "Cluster without Genes of Interest"
  ...
  if (is.null(plot.coord)) {
    plot.coord = data.frame(igraph::layout.fruchterman.reingold(module.hierarchy))
    colnames(plot.coord) = c("X1","X2")
    rownames(plot.coord) <- V(module.hierarchy)$name
  } else {
    plot.coord <- data.frame(plot.coord[match(V(module.hierarchy)$name,rownames(plot.coord)),])
  }
  
  module.vsize = ifelse(V(module.hierarchy)$name %in% module.table$id, module.table$module.size, max(module.table$module.size,na.rm = TRUE))
  ...
  node.features$node_color <- module.table$Module_Color[match(node.features$id, module.table$id)]
  ...
  hierarchy.obj <- ggplot() + 
    ...
    geom_point(data=node.features,mapping = aes(x = X1,y = X2,size = node_size * node.scaleFactor,colour = node_color)) +
    geom_text_repel(data = node.features,aes(x = X1,y = X2,label = node.lab,size = node_size * label.scaleFactor),colour = node.label.color,show.legend = FALSE, 
                    nudge_x = 0.5, nudge_y = 0.5, force = 10, max.iter = 1000, box.padding = 0.5, point.padding = 0.5)+
    theme_minimal() + 
