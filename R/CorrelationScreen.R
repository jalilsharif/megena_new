                                 ...
                                 rho.thresh = NULL,sort.el = TRUE,n.cores = n.cores)
{
  ...
  # Calculate rho.cutoff based on direction
  if (direction %in% c("absolute", "positive", "negative")) {
    is_min <- direction %in% c("absolute", "positive")
    rho.cutoff <- ifelse(is_min, min, max)(FDR.table$cut.off[FDR.table$FDR < FDR.cutoff])
  }
  
  
  # Coerce into edgelist
  ij <- which(direction %in% c("absolute", "positive")
              & rho > rho.cutoff & upper.tri(rho) |
                direction == "negative" & rho <= rho.cutoff & upper.tri(rho), arr.ind = TRUE)
  w <- rho[ij]
  
  ijw <- cbind(ij, w)
  colnames(ijw) <- c("row", "col", "rho")
  ...
  if (sort.el) {
    ijw <- ijw[order(ijw[, 3], decreasing = direction %in% c("absolute", "positive")), ]
  }
...
calculate.correlation <- function(datExpr,doPerm = 100,doPar = FALSE,n.cores = n.cores,method = "pearson",
                                  use.obs = "na.or.complete",
                                  ...
                                  output.permFDR = TRUE,output.corTable = TRUE,saveto = NULL)
{
  ...
  if (doPerm == 0)
  {
    ...
    rho.output <- calculate.rho.signed(datExpr,n.perm = doPerm,FDR.cutoff = FDR.cutoff,estimator = method,use.obs = use.obs,
                                       rho.thresh =  seq(0,1,1/n.increment),sort.el = TRUE,n.cores = n.cores)
